...
class CoordinatorLogin extends StatefulWidget {
  const CoordinatorLogin({super.key});

  @override
  State<CoordinatorLogin> createState() => _CoordinatorLoginState();
}

class _CoordinatorLoginState extends State<CoordinatorLogin> {
  ...
  logIn() async {
    ...
    try {
      ...
      await prefs.setString('mailId', mailController.text.toString().toLowerCase().replaceAll("@festum.in", ""));
      // await prefs.setString('rollNumber', mailController.text.toString().replaceAll("@festum.in", ""));

      await prefs.setInt('lType',4);
